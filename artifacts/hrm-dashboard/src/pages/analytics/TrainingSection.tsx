...
import { TRAINING_BY_DEPT, OVERVIEW_METRICS } from "@/data/demo";
...
export default function TrainingSection() {
  ...
  const statusData = [
    ...
    { label: t("completed"), count: m.completedTraining, pct: +((m.completedTraining / total) * 100).toFixed(1), color: "#10B981" },
  ];
  ...
  return (
    <section>
      <SectionHeader title={t("section_training_title")} desc={t("section_training_desc")} />

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-card border border-border rounded-xl p-6">
          <h3 className="font-semibold text-sm text-foreground mb-0.5">{t("chart_training_by_dept")}</h3>
        ...
        <div className="bg-card border border-border rounded-xl p-6 flex flex-col">
          <h3 className="font-semibold text-sm text-foreground mb-0.5">{t("chart_training_status")}</h3>
          <p className="text-xs text-muted-foreground mb-6">
            {t("chart_training_status_desc_pre")} {total} {t("chart_training_status_desc_post")}
          </p>
          ...
          <div className="mt-6 pt-4 border-t border-border space-y-2">
            <p className="text-xs font-semibold uppercase tracking-widest text-muted-foreground mb-2">{t("key_insights")}</p>
