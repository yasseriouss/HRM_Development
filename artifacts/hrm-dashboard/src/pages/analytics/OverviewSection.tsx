...
import { OVERVIEW_METRICS, CAMPAIGNS } from "@/data/demo";
...
export default function OverviewSection() {
  ...
  const campaignBar = [
    { status: t("chart_campaign_status").split(" ")[0], count: m.activeCampaigns },
    { status: t("completed"), count: m.completedCampaigns },
  ];

  return (
    <section>
      <SectionHeader title={t("section_overview_title")} desc={t("section_overview_desc")} />

      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        <KpiCard label={t("kpi_total_employees")} value={m.totalEmployees} sub={t("kpi_active_staff")} color="#D4960A" />
        <KpiCard label={t("kpi_departments")} value={m.totalDepartments} sub={t("kpi_wood_units")} />
        <KpiCard label={t("kpi_skills_tracked")} value={m.totalSkills} sub={t("kpi_across_all_depts")} />
        <KpiCard label={t("kpi_avg_score")} value={`${m.avgSkillPct}%`} sub={t("kpi_company_wide")} color="#10B981" />
      </div>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-card border border-border rounded-xl p-6">
          <h3 className="font-semibold text-sm text-foreground mb-0.5">{t("chart_perf_dist")}</h3>
          <p className="text-xs text-muted-foreground mb-4">{t("chart_perf_dist_desc")}</p>
        ...
        <div className="bg-card border border-border rounded-xl p-6 flex flex-col items-center justify-center">
          <h3 className="font-semibold text-sm text-foreground mb-0.5 self-start">{t("chart_avg_skill_score")}</h3>
          <p className="text-xs text-muted-foreground mb-2 self-start">{t("chart_avg_skill_score_desc")}</p>
          <GaugeChart pct={m.avgSkillPct} label={t("chart_company_avg")} />
        ...
        <div className="bg-card border border-border rounded-xl p-6">
          <h3 className="font-semibold text-sm text-foreground mb-0.5">{t("chart_campaign_status")}</h3>
          <p className="text-xs text-muted-foreground mb-4">{t("chart_campaign_status_desc")}</p>
          ...
          <div className="mt-4 space-y-3">
            {[
              { label: t("pending_training"), val: m.pendingTraining, color: "#EF4444" },
              { label: "In Progress", val: m.inProgressTraining, color: "#EAB308" },
              { label: t("completed"), val: m.completedTraining, color: "#10B981" },
            ].map((s, i) => (
