...
import { DEPT_PERFORMANCE } from "@/data/demo";
...
export default function DeptSection() {
  ...
  return (
    <section>
      <SectionHeader title={t("section_dept_title")} desc={t("section_dept_desc")} />

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-card border border-border rounded-xl p-6">
          <h3 className="font-semibold text-sm text-foreground mb-0.5">{t("chart_score_ranking")}</h3>
          <p className="text-xs text-muted-foreground mb-4">{t("chart_score_ranking_desc")}</p>
          <ResponsiveContainer width="100%" height={280}>
            <BarChart data={barData} layout="vertical" margin={{ top: 0, right: 60, left: 0, bottom: 0 }} barSize={18}>
              ...
              <Tooltip
                ...
                formatter={(v: number) => [`${v}%`, t("kpi_avg_score")]}
        ...
        <div className="bg-card border border-border rounded-xl p-6 overflow-auto">
          <h3 className="font-semibold text-sm text-foreground mb-0.5">{t("chart_comparison_table")}</h3>
          <p className="text-xs text-muted-foreground mb-4">{t("chart_comparison_table_desc")}</p>
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border">
                <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-widest text-muted-foreground">{t("col_department")}</th>
                {th(t("col_employees"), "employees")}
                {th(t("col_avg_pct"), "avgPct")}
                {th(t("col_class_a"), "classA")}
                {th(t("col_class_b"), "classB")}
                {th(t("col_class_c"), "classC")}
              </tr>
            </thead>
            <tbody>
              {sorted.map((d) => (
