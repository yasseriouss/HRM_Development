...
import { fetchWorkflows, fetchMyTasks, WorkflowInstance, WorkflowTask } from "@/lib/api";

const MAIN_APP_TASKS_URL = "/ebdaa-skill-matrix/my-tasks";
...
export default function WorkflowSummaryWidget() {
  ...
  const activeLabel = t("chart_workflow_active");
  const finalizedLabel = t("chart_workflow_finalized");
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-4 mb-5 flex-wrap">
        <div>
          <h3 className="font-semibold text-sm text-foreground mb-0.5">{t("widget_workflows_title")}</h3>
          <p className="text-xs text-muted-foreground">{t("widget_workflows_desc")}</p>
        </div>
        <span
      ...
      {loading ? (
        ...
        <div className="py-4 text-xs text-muted-foreground">{t("widget_error")}</div>
      ) : (
        <>
          <div className="flex gap-3 flex-wrap mb-6">
            <StatPill
              value={activeCount}
              label={t("widget_active_workflows")}
              color={GOLD}
            />
            <StatPill
              value={completedCount}
              label={t("widget_completed_workflows")}
              color={GREEN}
            />
            <StatPill
              value={pendingCount}
              label={t("widget_pending_approval")}
      ...
      {!loading && !error && pendingCount > 0 && (
        ...
        >
          ...
          {pendingCount} {t("widget_pending_approval").toLowerCase()} — {t("widget_view_tasks")}
        </div>
      )}
    </a>
  );
