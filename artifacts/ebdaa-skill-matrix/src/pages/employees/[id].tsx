...
export default function EmployeeProfilePage() {
  ...
  const { data: profile, isLoading } = useGetEmployeeProfile(id, { request: { headers } });
  ...
  if (!profile) return <div className="text-muted-foreground">{t("emp_not_found")}</div>;

  const data = profile as EmployeeProfile;
  const emp = data.employee;
  const latestSummary = data.latest_summary;
  const skillScores = data.skill_scores ?? [];
  const training = data.training_recommendations ?? [];
  const historicalSummaries: EvaluationSummary[] = data.historical_summaries ?? [];
  ...
  return (
    <div className="space-y-6">
      ...
      >
        <ArrowLeft className="h-4 w-4" /> {t("emp_back")}
      </Link>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center gap-4">
            ...
            {latestSummary && (
              <div className="text-end">
                ...
                <p className="text-xs text-muted-foreground">{t("emp_latest_score")}</p>
              </div>
            )}
          </div>
        </CardContent>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <Card className="border-border">
          <CardHeader>
            <CardTitle className="text-sm">{t("emp_skill_competencies")}</CardTitle>
          </CardHeader>
          <CardContent className="space-y-3">
            {skillScores.length === 0 ? (
              <p className="text-sm text-muted-foreground">{t("emp_no_skills")}</p>
            ) : (
              skillScores.map((sk) => (
                <div key={sk.skill_id} className="space-y-1">
                  <div className="flex justify-between text-xs">
                    ...
                    <span className="text-muted-foreground ms-2">
                      {sk.score != null ? t(scoreLabels[sk.score]) : t("emp_not_evaluated")}
                    </span>
                  </div>
        ...
        <div className="space-y-6">
          <Card className="border-border">
            <CardHeader>
              <CardTitle className="text-sm">{t("emp_eval_history")}</CardTitle>
            </CardHeader>
            <CardContent>
              {historicalSummaries.length === 0 ? (
                <p className="text-sm text-muted-foreground">{t("emp_no_evals")}</p>
              ) : (
          ...
          {training.length > 0 && (
            <Card className="border-border">
              <CardHeader>
                <CardTitle className="text-sm">{t("emp_training_recs")}</CardTitle>
              </CardHeader>
