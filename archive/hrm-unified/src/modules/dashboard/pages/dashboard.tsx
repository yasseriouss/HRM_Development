...
export default function Dashboard() {
    ...
    return (
      <div className="min-h-screen bg-[#040404] flex items-center justify-center p-6 industrial-grid">
        <div className="hacker-loader">
          <div className="loader-text"><span className="text-glitch" data-text={t('common_loading')}>{t('common_loading')}</span></div>
    ...
    return (
      <div className="min-h-screen bg-[#040404] flex items-center justify-center p-6 industrial-grid">
        <Card className="bg-[#0A0A0A] border-2 border-destructive/20 rounded-none p-12 text-center max-w-md relative">
          ...
          <p className="text-destructive font-headline font-black uppercase tracking-widest mb-2">{t('dash_error_loading')}</p>
          <p className="text-xs text-zinc-500 font-mono mb-8 uppercase leading-relaxed">{error}</p>
          <Button onClick={logout} variant="outline" className="rounded-none border-destructive/30 text-destructive hover:bg-destructive/10 uppercase font-black text-[10px] tracking-widest">{t('dash_error_sign_out')}</Button>
  ...
  return (
    <div className="min-h-full text-foreground selection:bg-primary selection:text-primary-foreground industrial-grid">
      <main className="max-w-7xl mx-auto px-8 py-12 space-y-12 pb-32">
        ...
        >
          <div className="flex items-center gap-3">
             ...
             <h2 className="text-4xl font-headline font-black text-white uppercase tracking-tighter">{t('dash_workforce_overview')}</h2>
          </div>
          <p className="text-xs text-zinc-500 font-mono uppercase tracking-[0.2em]">{t('dash_workforce_subtitle')}</p>
        </motion.div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <StatCard label="Active Campaigns" value={metrics?.active_campaigns ?? 0} sub={`${metrics?.completed_campaigns ?? 0} ${t('dash_completed')}`} icon={Zap} colorClass="amber-500" />
          <StatCard label={t('dash_kpi_skills_tracked')} value={metrics?.total_skills ?? 0} sub="competency nodes" icon={Target} colorClass="emerald-500" />
          <StatCard label={t('dash_kpi_avg_score')} value={metrics?.average_skill_percentage != null ? `${metrics.average_skill_percentage}%` : "—"} sub="global baseline" icon={Activity} colorClass="blue-500" />
        </div>
            ...
            <CardContent className="p-8">
              {classDistData.length > 0 ? (
                ...
                <div className="h-[260px] flex flex-col items-center justify-center text-center p-12 border border-dashed border-zinc-900">
                  ...
                  <p className="text-zinc-600 font-mono text-[10px] uppercase tracking-widest">{t('dash_no_eval_data')}</p>
                </div>
              )}
            </CardContent>
            <CornerMarks />
          ...
          <Card className="bg-[#0A0A0A] border border-zinc-900 rounded-none relative overflow-hidden group">
            ...
            <CardContent className="p-8 space-y-8">
              ...
              <div className="pt-6 border-t border-zinc-900 flex justify-between items-center">
                  <span className="text-[9px] font-black text-zinc-600 uppercase tracking-widest">{t('dash_pending_training')}</span>
        ...
        <Card className="bg-[#0A0A0A] border border-zinc-900 rounded-none relative overflow-hidden group">
          ...
          <CardContent className="p-8">
            {deptChartData.length > 0 ? (
              ...
              <div className="h-[320px] flex items-center justify-center text-zinc-700 font-mono text-[10px] uppercase tracking-widest">{t('dash_no_dept_data')}</div>
            )}
          </CardContent>
          <CornerMarks />
        </Card>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <Card className="bg-[#0A0A0A] border border-zinc-900 rounded-none relative overflow-hidden group">
            <CardHeader className="p-8 border-b border-zinc-900/50">
              <CardTitle className="font-headline font-black text-sm uppercase tracking-widest text-zinc-400">{t('dash_recent_activity')}</CardTitle>
            </CardHeader>
            <CardContent className="p-8 space-y-6">
              {activity.length > 0 ? activity.map((item) => (
                ...
                <p className="text-[10px] font-mono text-zinc-700 uppercase tracking-widest">{t('dash_no_recent_activity')}</p>
              )}
            </CardContent>
          ...
          <Card className="bg-[#0A0A0A] border border-zinc-900 rounded-none relative overflow-hidden group">
            <CardHeader className="p-8 border-b border-zinc-900/50">
              <CardTitle className="font-headline font-black text-sm uppercase tracking-widest text-zinc-400">{t('dash_avg_score_by_dept')}</CardTitle>
            </CardHeader>
            <CardContent className="p-8 space-y-6">
              {deptPerf.length > 0 ? [...deptPerf].sort((a, b) => b.average_percentage - a.average_percentage).map((dept) => (
                ...
                <p className="text-[10px] font-mono text-zinc-700 uppercase tracking-widest">{t('dash_no_data')}</p>
              )}
            </CardContent>
            <CornerMarks color="zinc-800" />
          </Card>
        ...
        <footer className="text-center pt-12 border-t border-zinc-900/50">
          <p className="text-[9px] font-black text-zinc-600 uppercase tracking-[0.4em]">
            {t('dash_app_footer')} · {t('dash_app_created_by')} <span className="text-primary hover:text-white transition-colors cursor-pointer">yasserious.com</span>
          </p>
        </footer>
      </main>
    </div>
