...
import { EMPLOYEE_DATA, CLASS_PROGRESSION } from "@modules/dashboard/data/demo";
...
export default function EmployeeSection() {
  ...
  return (
    <section>
      <SectionHeader title={t('dash_section_employee_title')} desc={t('dash_section_employee_desc')} />

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-card border border-border rounded-xl p-6">
          <h3 className="font-semibold text-sm text-foreground mb-0.5">{t('dash_chart_emp_scatter')}</h3>
          <p className="text-xs text-muted-foreground mb-4">{t('dash_chart_emp_scatter_desc')}</p>
        ...
        <div className="bg-card border border-border rounded-xl p-6 flex flex-col">
          <h3 className="font-semibold text-sm text-foreground mb-0.5">{t('dash_chart_class_prog')}</h3>
          <p className="text-xs text-muted-foreground mb-6">{t('dash_chart_class_prog_desc')}</p>

          <div className="flex-1 space-y-4">
            {[
              { label: t('dash_prog_improved'), desc: t('dash_prog_improved_desc'), count: CLASS_PROGRESSION.improved, color: CLR.A, icon: "↑" },
              { label: t('dash_prog_maintained'), desc: t('dash_prog_maintained_desc'), count: CLASS_PROGRESSION.maintained, color: "#9AA0AE", icon: "→" },
              { label: t('dash_prog_declined'), desc: t('dash_prog_declined_desc'), count: CLASS_PROGRESSION.declined, color: CLR.C, icon: "↓" },
            ].map((p, i) => (
          ...
          <div className="mt-4 rounded-lg p-3 text-xs" style={{ background: "rgba(212,150,10,0.08)", border: "1px solid rgba(212,150,10,0.2)" }}>
            <p className="text-foreground font-medium mb-1">{t('dash_net_improvement')}</p>
            ...
            <p className="text-muted-foreground">{t('dash_net_improvement_sub')}</p>
          </div>
        </div>
      </div>
    </section>
