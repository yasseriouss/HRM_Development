...
export default function Slide14_Metrics() {
  ...
  return (
    <div className="relative w-screen h-screen overflow-hidden" style={{ background: "var(--slide-bg)" }}>
      ...
      <div className={`absolute top-[7vh] ${isAr ? 'right-[8vw]' : 'left-[8vw]'} met-h`} style={{ textAlign: isAr ? 'right' : 'left' }}>
      ...
      <div className={`absolute top-[22vh] ${isAr ? 'right-[8vw]' : 'left-[8vw]'} ${isAr ? 'left-[6vw]' : 'right-[6vw]'} met-grid`} style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: "2vw", direction: isAr ? 'rtl' : 'ltr' }}>
        {metrics.map((m, i) => (
          <div key={i} style={{ background: m.bg, border: `0.15vw solid ${m.border}`, borderRadius: "0.8vw", padding: "3vh 2.5vw", textAlign: isAr ? 'right' : 'left' }}>
