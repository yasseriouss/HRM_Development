    ...
    <div className="max-w-7xl mx-auto space-y-12 py-16 px-8 pb-32 selection:bg-zinc-900 selection:text-white">
      ...
      <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-12">
        <div className="space-y-6">
           ...
           <p className="text-zinc-500 font-medium text-lg max-w-2xl leading-relaxed">{t("je_analytics_subtitle")}</p>
        </div>

        <div className="flex flex-wrap items-center gap-4">
          <Button variant="outline" className="rounded-full border-zinc-100 bg-white text-zinc-900 font-bold text-[10px] tracking-widest uppercase h-16 px-10 shadow-sm hover:shadow-xl transition-all">
            <FileDown className="h-4 w-4 me-3" /> EXPORT ANALYTICS
          </Button>
          <Button className="rounded-full bg-zinc-900 text-white font-bold text-[10px] tracking-widest uppercase h-16 px-10 shadow-2xl shadow-zinc-200 hover:scale-[1.02] transition-all">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
        {[
          { label: t("je_total_jobs"), value: "124", icon: Briefcase, color: "text-zinc-900", bg: "bg-zinc-50" },
          ...
          { label: "Active Nodes", value: "852", icon: Users, color: "text-zinc-400", bg: "bg-zinc-50" },
        ].map((m, i) => (
          <motion.div
            key={i}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
        ...
        <Card className="lg:col-span-8 bg-white border-zinc-100 rounded-4xl p-12 shadow-sm hover:shadow-xl transition-all duration-500">
           <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-8 mb-12">
              <div className="space-y-3">
                 <h2 className="text-3xl font-bold font-comfortaa tracking-tighter text-zinc-900 uppercase">Equity Correlation</h2>
                 <p className="text-xs text-zinc-400 font-bold uppercase tracking-widest">Points to Salary Distribution Matrix</p>
              </div>
              <Badge className="rounded-full bg-zinc-900 text-white border-none text-[9px] font-bold tracking-widest uppercase px-6 py-2">
                 R² = 0.942
              </Badge>
           </div>
           ...
           <div className="h-[400px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                 <ScatterChart margin={{ top: 20, right: 20, bottom: 20, left: 20 }}>
                    ...
                    <Tooltip 
                      ...
                      content={({ active, payload }) => {
                        if (active && payload && payload.length) {
                          return (
                            <div className="bg-zinc-900 text-white p-6 rounded-3xl shadow-2xl border-none">
                              ...
                              <p className="text-xs font-bold text-zinc-400">{payload[0].value} POINTS</p>
                            </div>
                          );
                        }
                        return null;
                    ...
                    />
                    <Scatter name="Jobs" data={scatterData} fill="#18181B">
        ...
        <Card className="lg:col-span-4 bg-zinc-900 border-none rounded-4xl p-12 shadow-2xl shadow-zinc-200 relative overflow-hidden group">
           <div className="absolute inset-0 bg-[radial-gradient(circle_at_70%_30%,#3F3F46_0%,transparent_60%)] opacity-20" />
           <div className="relative z-10 space-y-12">
              <div className="space-y-3">
                 <h2 className="text-3xl font-bold font-comfortaa tracking-tighter text-white uppercase">{t("je_grade_dist")}</h2>
                 <p className="text-xs text-white/40 font-bold uppercase tracking-widest">Global Structural Hierarchy</p>
              </div>
                    ...
                    <PieChart>
                       ...
                       >
                          {gradeDist.map((entry, index) => (
                            <Cell key={`cell-${index}`} fill={index === 0 ? '#FFFFFF' : `rgba(255,255,255,${0.4 - (index * 0.1)})`} stroke="none" />
                          ))}
                       </Pie>
                    </PieChart>
                 </ResponsiveContainer>
                 <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                    <span className="text-3xl font-bold font-comfortaa text-white">100%</span>
                 ...
                 {gradeDist.map((g, i) => (
                   <div key={i} className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                         <div className="h-2 w-2 rounded-full" style={{ backgroundColor: i === 0 ? '#FFFFFF' : `rgba(255,255,255,${0.4 - (i * 0.1)})` }} />
                         <span className="text-[10px] font-bold text-white/60 uppercase tracking-widest">{g.name}</span>
                      </div>
                      <span className="text-sm font-bold font-comfortaa text-white">{g.value}%</span>
                   </div>
                 ))}
              </div>
           </div>
      ...
      <Card className="bg-white border-zinc-100 rounded-4xl shadow-sm overflow-hidden">
        <CardContent className="p-8 flex flex-col sm:flex-row items-center gap-8">
           ...
           <Button variant="outline" className="h-16 rounded-3xl border-zinc-100 px-8 gap-3 text-zinc-400 hover:text-zinc-900">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-12 pt-10">
         <motion.div whileHover={{ scale: 1.02 }} className="cursor-pointer group">
            <Card className="bg-white border-zinc-100 rounded-4xl p-12 shadow-sm group-hover:shadow-2xl group-hover:shadow-zinc-100 transition-all duration-500 relative overflow-hidden">
               <div className="absolute top-0 right-0 p-10 opacity-5 group-hover:opacity-20 transition-all">
                  <Cpu className="h-16 w-16 text-zinc-900" />
               </div>
               <div className="space-y-6">
                  <div className="w-16 h-16 rounded-3xl bg-zinc-50 flex items-center justify-center text-zinc-900 group-hover:bg-zinc-900 group-hover:text-white transition-all duration-500">
                     <ShieldCheck className="h-8 w-8" />
                  </div>
                  <div>
                     <h4 className="text-3xl font-bold font-comfortaa text-zinc-900 tracking-tighter uppercase">Review Methodologies</h4>
                     <p className="text-xs text-zinc-400 font-bold uppercase tracking-widest mt-2 leading-relaxed">Update points allocation and scoring criteria</p>
                  </div>
                  <div className="flex items-center gap-2 text-zinc-300 group-hover:text-zinc-900 transition-colors">
         ...
         <motion.div whileHover={{ scale: 1.02 }} className="cursor-pointer group">
            <Card className="bg-zinc-900 border-none rounded-4xl p-12 shadow-2xl shadow-zinc-200 group-hover:scale-[1.03] transition-all duration-500 relative overflow-hidden">
               <div className="absolute inset-0 bg-[radial-gradient(circle_at_0%_100%,#3F3F46_0%,transparent_50%)] opacity-20" />
               <div className="space-y-6 relative z-10">
                  <div className="w-16 h-16 rounded-3xl bg-white/10 flex items-center justify-center text-white">
                     <Plus className="h-8 w-8" />
                  </div>
                  <div>
                     <h4 className="text-3xl font-bold font-comfortaa text-white tracking-tighter uppercase">Benchmarking</h4>
                     <p className="text-xs text-white/40 font-bold uppercase tracking-widest mt-2 leading-relaxed">Compare internal data with market standards</p>
                  </div>
