...
export default function DocsModule() {
  ...
  return (
    <div className="flex w-full min-h-screen">
      <aside id="sidebar" className={sidebarOpen ? "open" : ""}>
        <div className="nav-header">
          <div className="nav-logo">
            <div className="nav-logo-mark">HZ</div>
            <div>
              <div className="nav-logo-text">{t("docs_app_title")}</div>
              <div className="nav-sub">{t("docs_app_subtitle")}</div>
            </div>
          </div>
          <div className="nav-credit">
            {t("docs_created_by")} <a href="https://yasserious.com" target="_blank" rel="noopener noreferrer">yasserious.com</a>
          </div>
        </div>
            ...
            {searchOpen && (
              <div className="search-results">
                {searchResults.length === 0 ? (
                  <div className="sr-empty">{t("docs_no_results")} "{searchQuery}"</div>
                ) : (
                  searchResults.map((r) => (
      ...
      <div id="main-wrap">
        <main ref={mainRef} dangerouslySetInnerHTML={{ __html: lang === "ar" ? docContentAr : docContentEn }} />
      </div>
    </div>
  );
}
